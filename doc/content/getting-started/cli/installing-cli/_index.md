...
> When installing with `brew`, auto-completion is enabled automatically.
...
> When installing with `snap`, auto-completion is enabled automatically.
...
> When using the snap packages, `~/.ttn-lw-cli.yml` will fail with permission errors. Choose a different path.
...
> On Windows, use `ttn-lw-cli.exe` instead of `ttn-lw-cli`.
...
> You can also use the `--grpc-port` and `--oauth-server-address` flags to override the default values for the gRPC port and the OAuth server address. These are not needed for standard deployments.
